...
const getWorkouts = async (req, res) => { try { const userId = req.query.userId; const workouts = await Workout.find({ userId: userId }).sort({ createdAt: -1 }); res.status(200).json(workouts);} catch (error) { res.status(500).json({ message: 'Error fetching workouts', error: error.message }); } };
...
// עדכון הפונקציה ב-workoutController.js
const getWorkoutStats = async (req, res) => {
    try {
        // משיכת ה-userId מה-query שהלקוח שולח
        ...
        // הוספנו $match עם userId לכל השאילתות
        const dailyStats = await Workout.aggregate([ 
            { $match: { userId: userId, createdAt: { $gte: startOfToday } } }, 
            { $group: { _id: null, steps: { $sum: "$steps" }, calories: { $sum: "$calories" }, duration: { $sum: "$duration" } }} 
        ]);
        
        const weeklyStats = await Workout.aggregate([ 
            { $match: { userId: userId, createdAt: { $gte: startOfWeek } } }, 
            { $group: { _id: null, totalWorkouts: { $sum: 1 }, totalDistance: { $sum: "$distance" }, totalCalories: { $sum: "$calories" }, totalDuration: { $sum: "$duration" } }} 
        ]);
...
// 7. גרפים (מתוקן עם סינון לפי userId)
const getWeeklyChartData = async (req, res) => {
    try {
        const userId = req.query.userId; // תוספת: משיכת ה-ID
        const sevenDaysAgo = new Date();
        sevenDaysAgo.setDate(sevenDaysAgo.getDate() - 7);
        
        // תוספת: { $match: { userId: userId, ... } }
...
const getMonthlyChartData = async (req, res) => {
    try {
        ...
        const chartData = await Workout.aggregate([ 
            { $match: { userId: userId } },
            { $group: { _id: { $isoWeek: "$createdAt" }, totalCalories: { $sum: "$calories" } } }, 
            { $sort: { "_id": -1 } }, 
            { $limit: 4 } 
        ]);
